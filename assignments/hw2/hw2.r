...
#wr1500m$times == min(wr1500m$times)

#wr.name <- wr1500m[max(1:51), 3:4]

#wr.name <- min(wr1500$times)
...
lines(x = c(1998, 2014), y = rep(min(wr1500m$times_sec),2))
...
text(x = wr_1944, y = wr1500m$times_sec[wr1500m$new_year == wr_1944], 
     label = wr1500m$athlete[wr1500m$new_year == wr_1944],
     adj = 1, cex = 1, col = "red")
text(wr_1998, wr1500m$times_sec[wr1500m$new_year == wr_1998], 
     label = wr1500m$athlete[wr1500m$new_year == wr_1998],
     adj = 1, cex = 1, col = "red")
...
plot(wr1500m$new_year, wr1500m$times_sec,  type = "s", 
     xlim = c(min(wr1500m$new_year), max(wr1500m$new_year) + 17), xlab = "Year",
     ylab = "Times (seconds)", main = "Men's World Records for 1500 meter")
...
plot(SO2012Ctry$GDP, SO2012Ctry$pop, 
     xlab = "GDP", ylab = "Population", main = "GDP v. Population")
...
symbols(log(SO2012Ctry$pop), log(SO2012Ctry$GDP_per_person), 
        circles = log(SO2012Ctry$Total)/25, inches = FALSE)
...
with(SO2012Ctry[SO2012Ctry$Total == 0, ], 
     points(SO2012Ctry$pop), log(SO2012Ctry$GDP_per_person), pch = ".")
...
symbols(log(SO2012Ctry$pop), log(SO2012Ctry$GDP_per_person), 
        circles = sqrt(SO2012Ctry$Total)/34, inches = FALSE,
        xlab = "population (log)", ylab = "GDP per person (log)", 
