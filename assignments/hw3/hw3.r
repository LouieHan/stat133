...
qplot(wr1500m$new_year, wr1500m$times_sec, geom = "line")
...
qplot(wr1500m$new_year, wr1500m$times_sec, geom = "line", xlim = c(min(wr1500m$new_year), max(wr1500m$new_year) + 17)) +
  geom_segment(aes(x = max(wr1500m$new_year), y = min(wr1500m$times_sec), xend = 2014, yend = min(wr1500m$times_sec)))
...
wr_1998 <- wr1500m$new_year[wr1500m$times == min(wr1500m$times)]

# Your ggplot command
qplot(wr1500m$new_year, wr1500m$times_sec, geom = "line", xlim = c(min(wr1500m$new_year), max(wr1500m$new_year) + 17)) +
  geom_segment(aes(x = max(wr1500m$new_year), y = min(wr1500m$times_sec), xend = 2015, yend = min(wr1500m$times_sec))) +
  geom_vline(xintercept = wr_1944, col = "green") +
  geom_vline(xintercept = wr_1998, col = "green") +
  annotate("text", label = wr1500m$athlete[wr1500m$new_year == wr_1944],  x = wr_1944, y = wr1500m$times_sec[wr1500m$year == 1944] + 8, colour = "blue") +
  annotate("text", label = wr1500m$athlete[wr1500m$new_year == wr_1998],  x = wr_1998, y = wr1500m$times_sec[wr1500m$year == 1998] + 8, colour = "blue")
...
qplot(wr1500m$new_year, wr1500m$times_sec, geom = "line", xlim = c(min(wr1500m$new_year), max(wr1500m$new_year) + 17)) +
  ...
  annotate("text", label = wr1500m$athlete[wr1500m$new_year == wr_1944],  x = wr_1944, y = wr1500m$times_sec[wr1500m$year == 1944] + 8, colour = "blue") +
  annotate("text", label = wr1500m$athlete[wr1500m$new_year == wr_1998],  x = wr_1998, y = wr1500m$times_sec[wr1500m$year == 1998] + 8, colour = "blue") +
  labs(x = "Year", y = "Record Times", title = "World Record for Men 1500M")
...
qplot(log(SO2012Ctry$pop), log(SO2012Ctry$GDP_per_person)) +
  geom_point(aes(size = log(SO2012Ctry$Total)/25))
...
qplot(log(SO2012Ctry$pop), log(SO2012Ctry$GDP_per_person)) +
  geom_point(aes(size = log(SO2012Ctry$Total)/25)) +
  labs(x = "Population", y = "GDP per Person", title = "Medals by Population and GDP") +
  annotate("text", label = SO2012Ctry$ISO[top5],  x = log(SO2012Ctry$pop)[top5], y = log(SO2012Ctry$GDP_per_person)[top5] + .5, colour = "red")
...
blank <- ggplot()
graph1 <- blank + geom_polygon(data=world.map , aes(x=long, y=lat, group=group), fill="grey40")
graph1
...
wonMedal <- SO2012Ctry[SO2012Ctry$Total >0,][1:13]
# Your ggplot commands here.
graph2 <- graph1 + geom_point(aes(x=wonMedal$long, y=wonMedal$lat), size= sqrt(wonMedal$Total)/1.5,colour = "gold")
graph2
...
gender_plot <-ggplot(data=athletes, aes(Sport, fill = Sex))+
...
gender_plot2 <- gender_plot + labs(title= "Olympic Medals by Gender", x = "Olympic Sports", y= "# of Medals") + theme(axis.text.x = element_text(angle = 90)) 
gender_plot2
...
max.rain <- unlist(lapply(rain, function(x) x[which.max(x)]))
