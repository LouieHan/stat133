...
name <- "Ryan Lee"
github.name <- "LouieHan"
email.address <- "yourfriendryanlee@berkeley.edu"
...
# y <- <your code here>
l <- list(); for(i in 1:10) l[[i]] <- seq(from = i, to = 1000, by = i)
...
cw3 <- mean(ChickWeight[ ChickWeight$weight>41 & ChickWeight$Time == 0,1 ])

# [1 pt]
# Create [cw4] a random subset of 200 ChickWeight observations (any 200)
#cw4 <- <your code here>
  
sampler <- sample(1:578,200, FALSE)
cw4 <- ChickWeight[sample(1:578,200, FALSE),]
...
bdiff <- mean(na.omit(infants[infants$age > 35, 2])) - mean(na.omit(infants[infants$age < 35, 2]))
...
plot(mtcars$wt, mtcars$disp, xlab="Weight of Car in lb/1000", ylab="Engine Displacement in Cu. In.", col=mtcars$am+1)
...
plot(WorldBank$fertility.rate, WorldBank$life.expectancy, pch = ".", type = "p", xlab = "World Fertility Rate", ylab = "World Life Expectancy", col = WorldBank$region)
abline(v = 2.1)
...
hist(WorldBank$GDP[WorldBank$lending == "IDA"], breaks = 250, xlab = "GDP for Countries with Lending Rate IDA", main = "Histogram of Countries with Lending Rate IDA")
...
max.rain <- NULL
max.rain[1] <- max(rain[[1]])
max.rain[2] <- max(rain[[2]])
max.rain[3] <- max(rain[[3]])
max.rain[4] <- max(rain[[4]])
max.rain[5] <- max(rain[[5]])
...
prop.rain <- NULL
prop.rain[1] <- sum(rain[[1]] > 0)
prop.rain[2] <- sum(rain[[2]] > 0)
prop.rain[3] <- sum(rain[[3]] > 0)
prop.rain[4] <- sum(rain[[4]] > 0)
prop.rain[5] <- sum(rain[[5]] > 0)
...
PermDiff <- function(cases, controls, k = 5000){
  case_prime <-sample(cases + controls, length(cases), FALSE)
  mean_case_prime <- mean(beep)
  return(mean_case_prime)
}
#??????????????????????????????????????????????????????????????????????/
...
truth <- NULL
truth[1] <- length(dna[1])%%3 == 0
truth[2] <- length(dna[2])%%3 == 0
truth[3] <- length(dna[3])%%3 == 0
truth[4] <- length(dna[4])%%3 == 0
truth[5] <- length(dna[5])%%3 == 0
truth[6] <- length(dna[6])%%3 == 0
truth[7] <- length(dna[7])%%3 == 0
truth[8] <- length(dna[8])%%3 == 0
truth[9] <- length(dna[9])%%3 == 0
dna2 <- dna == truth
#################################################################
